...
export class MerchantsService {
  private readonly logger = new Logger(MerchantsService.name); // Move logger initialization outside the constructor
  ...
  async deleteFile(fileName: string): Promise<void> {
    ...
    unlink(filePath, (err) => {
      if (err) {
        this.logger.error('Error deleting file:', err); // Use logger for error
        throw new Error('File deletion failed');
      }
      this.logger.log('File deleted successfully'); // Use logger for success message
    });
  }
